...
from timm.bits import initialize_device, setup_model_and_optimizer, DeviceEnv, DeviceEnvCuda, Monitor, Tracker,\
...
from timm.scheduler import create_scheduler, convert_scheduler_from_epoch_to_iter
from timm.utils import setup_default_logging, random_seed, set_up_deterministic_seed, get_outdir, unwrap_model
...
parser.add_argument('--step-in-iter', default=False, action='store_true',
                    help='to step shceduler per step or per epoch"')
parser.add_argument('--lr', type=float, default=0.1, metavar='LR',
                    help='learning rate (default: 0.05)')
parser.add_argument('--scale-lr',default=False, action='store_true',
                    help='lr = args.lr * args.batch_size * dev_env.world_size / 512.0')
parser.add_argument('--scale-all-lr',default=False, action='store_true',
                    help='lr = args.lr * args.batch_size * dev_env.world_size / 512.0 for base_lr, warmup_lr, min_lr')
...
##############
parser.add_argument('--warmup-prefix', type=str, default='none', choices=['none', 'step', 'all'],
                    help='choose how to calculate steps when using warmup with (cosine) lr scheduler.')
##############
...
parser.add_argument('--checkpoint-interval', type=int, default=5, metavar='N',
                    help='how many epochs to wait before writing checkpoint')
...
def main():
    ...
    ###############
    assert not (args.scale_lr and args.scale_all_lr)
    if args.scale_all_lr:
        linear_scaled_lr = args.lr * args.batch_size * dev_env.world_size / 512.0
        linear_scaled_warmup_lr = args.warmup_lr * args.batch_size * dev_env.world_size / 512.0
        linear_scaled_min_lr = args.min_lr * args.batch_size * dev_env.world_size / 512.0
    ...
    #############
    if args.step_in_iter:
        train_state.lr_scheduler = convert_scheduler_from_epoch_to_iter(train_state.lr_scheduler,
                                                                        len(loader_train) // args.update_freq)
    ############
    ...
    try:
        for epoch in range(train_state.epoch, train_cfg.num_epochs):
            ...
            if checkpoint_manager is not None:
                ...
                best_checkpoint = checkpoint_manager.save_checkpoint(train_state,
                                                                     save_epoch=(epoch + 1) % args.checkpoint_interval == 0,
...
def setup_data(args, default_cfg, dev_env: DeviceEnv, mixup_active: bool):
    ...
    _logger.info(f'len(dataset_train): {len(dataset_train)}')
    _logger.info(f'len(datset_eval): {len(dataset_eval)}')
...
def train_one_epoch(
    ...
    for step_idx, (sample, target) in enumerate(loader):
        ...
        #########
        state.updater.apply(loss / update_freq, accumulate=False if (step_idx+1) % update_freq == 0 else True)
...
def after_train_step(
    ...
    with torch.no_grad():
        ...
        if state.model_ema is not None and (step_idx+1) % update_freq == 0:
            # FIXME should ema update be included here or in train / updater step? does it matter?
            state.model_ema.update(state.model)

        if (step_idx+1) % update_freq == 0:
            # state = replace(state, step_count_global=state.step_count_global + 1)
            state.step_count_global = state.step_count_global + 1

        cfg = state.train_cfg

        if services is not None and services.monitor is not None and end_step or (step_idx + 1) % cfg.log_interval == 0:
        ...
        if services is not None and services.checkpoint is not None and cfg.recovery_interval and (
                end_step or (step_idx + 1) % cfg.recovery_interval == 0):
            services.checkpoint.save_recovery(state)

        # if state.lr_scheduler is not None:
        if state.lr_scheduler is not None and (step_idx + 1) % update_freq == 0:
...
def evaluate(
    ...
    with torch.no_grad():
        for step_idx, (sample, target) in enumerate(loader):
            ...
            if logger is not None and last_step or step_idx % log_interval == 0:
    ...
    top1, top5 = accuracy_m.compute().values()
    results = OrderedDict([(phase_suffix + 'loss', losses_m.compute().item()), (phase_suffix + 'top1', top1.item()), (phase_suffix + 'top5', top5.item())])
    return results
